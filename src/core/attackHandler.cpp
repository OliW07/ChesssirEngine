...
uint64_t AttackHandler::getAttacks(const int pos) {
    ...
    switch (pieceType) {
        case Pawn:
            ...
            PieceAttacks &=
                (game.board.isPieceWhite(pos) ? game.board.state.occupancy[Black] : game.board.state.occupancy[White]);
            break;
        case Knight:
    ...
    if (pieceType == Pawn && game.board.state.enPassantSquare != -1) {
        ...
        if ((game.board.isPieceWhite(pos) &&
        ...
        } else if (!game.board.isPieceWhite(pos) &&
                   ((game.board.state.enPassantSquare + 9 == pos) || (game.board.state.enPassantSquare + 7 == pos)) &&
                   !pieceWraps) {
            attacks |= (1ULL << game.board.state.enPassantSquare);
        }
...
uint64_t AttackHandler::getAttackers(int pos, bool attackingIsWhite) {
    ...
    attackers |= (attackingIsWhite ? (knightMoves[pos] & game.board.state.bitboards[White][Knight])
                                   : (knightMoves[pos] & game.board.state.bitboards[Black][Knight]));
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (game.board.isPieceWhite(attackingPiecePos) != attackingIsWhite)
            continue;
...
uint64_t AttackHandler::pawnControlledSquare(bool controllingColourIsWhite) {
    uint64_t pawnBitBoard =
        controllingColourIsWhite ? game.board.state.bitboards[White][Pawn] : game.board.state.bitboards[Black][Pawn];
...
uint64_t AttackHandler::getPinnedPieces(bool isWhite, bool includeEnemies) {
    ...
    for (RaysDirection direction : ALL_DIRECTIONS) {
        ...
        if (game.board.isPieceWhite(firstBlocker) != game.board.isPieceWhite(kingLocation) && !includeEnemies)
            continue;
        ...
        if (game.board.isPieceWhite(secondBlocker) == game.board.isPieceWhite(kingLocation))
            continue;
...
bool AttackHandler::isSquareAttacked(int pos, bool attackingColourIsWhite) {
    if (getPawnAttackers(pos, attackingColourIsWhite))
        return true;

    uint64_t enemyKnights =
        attackingColourIsWhite ? game.board.state.bitboards[White][Knight] : game.board.state.bitboards[Black][Knight];
    uint64_t enemyKing =
        attackingColourIsWhite ? game.board.state.bitboards[White][King] : game.board.state.bitboards[Black][King];
    uint64_t enemyRooks =
        attackingColourIsWhite ? game.board.state.bitboards[White][Rook] : game.board.state.bitboards[Black][Rook];
    uint64_t enemyBishops =
        attackingColourIsWhite ? game.board.state.bitboards[White][Bishop] : game.board.state.bitboards[Black][Bishop];
    uint64_t enemyQueens =
        attackingColourIsWhite ? game.board.state.bitboards[White][Queen] : game.board.state.bitboards[Black][Queen];
